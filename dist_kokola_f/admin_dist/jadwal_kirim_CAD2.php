            ...
            <div data-role="content">
                ...
                <table id="example" class="display" cellspacing="0" width="100%">
                    ...
                    while ($data_produk = mysqli_fetch_array($hasil_produk)) {
                        ...
                        /*cari akun id*/
                        $sql_akun_id = "SELECT  `order_kirim_wd.`ACCOUNT_ID`
                                                          FROM USER,`order_kirim_wd`,m_produk
                                                           WHERE user.`ACCOUNT_ID` = `order_kirim_wd`.`ACCOUNT_ID`
                                                           AND `m_produk`.`ITEM_CODE` = `order_kirim_wd`.`item_code`
                                                           AND order_kirim_wd.`periode2` = '$today_database'";

                          $hasil_akun_id = mysqli_query($mysqli, $sql_akun_id);
                        $data_akun_id = mysqli_fetch_assoc($hasil_akun_id);
                        $akun_id = $data_akun_id[ACCOUNT_ID];
                        ...
                        echo $akun_id;
                           ...
                           $sql_qty = "SELECT `order_kirim_wd`.qty
                                          FROM order_kirim_wd
                                           ...
                                           AND order_kirim_wd.periode2 = '$today_database'
                                           AND `order_kirim_wd`.`ACCOUNT_ID` = '$akun_id'";

                        echo $$sql_qty;

                                 $hasil_qty = mysqli_query($mysqli, $sql_qty);

                                 while($data_tulis_qty = mysqli_fetch_assoc($hasil_qty)){

                                      echo "<td>$data_tulis_qty[qty]</td>";
