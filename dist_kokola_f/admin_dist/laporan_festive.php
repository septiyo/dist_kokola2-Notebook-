...
$(document).ready(function () {
   $("td .std").click(function() {
	...
	var DIST = $(this).attr('man')
            window.open ( "detail_lap.php?INK="+DIST+"&POKU="+PO_KU, "MyWindow", 'width=1100, height=500, top=80, left=40, scrollbars=yes,resizable=yes, toolbar=no')	
...
<!--<div style="overflow-y:10px;">-->
 ...
 ( select * from
  (select TGL as TGLD, BULAN_INPUT, NAMA_PRODUK, ITEM_CODE as ICX,
         ...
         group by ACCOUNT_ID, YEAR(TGL)) as MIKA
  left join
  (select MAN.ACCOUNT_ID, SUM(MAN.REALX) as REALX, MAN.NAMA_PRODUK as NMP, MAN.ITEM_CODE from       
  (select SATU.TGL, SATU.NAMA_PRODUK, SATU.ITEM_CODE, SATU.ACCOUNT_ID,
         DUA.periode2, DUA.maxid, DUA.REALX, DUA.qty from 
 ( select * from kj_f) as SATU
 left join   
 (select *, SUM(CC.qty) as REALX from   
 (select * from order_kirim as AA, 
        (select  max(id) as maxid from order_kirim group by ACCOUNT_ID, YEAR(periode2), MONTH(periode2), item_code ) as BB 
         where  BB.maxid = AA.id) as CC
         group by  CC.ACCOUNT_ID, YEAR(CC.periode2), MONTH(CC.periode2), CC.item_code) as DUA
         on SATU.ACCOUNT_ID = DUA.ACCOUNT_ID and SATU.ITEM_CODE = DUA.item_code 
         and YEAR(SATU.TGL) = YEAR(DUA.periode2)) as MAN
         group by MAN.ACCOUNT_ID, YEAR(TGL)) as MIKI
         on MIKA.ACIDX = MIKI.ACCOUNT_ID) as SOGO
        left join
        (select ACCOUNT_ID as AI, ACCOUNT_NAME as ACN from push_distributor) as SIGI
        on SOGO.ACIDX = SIGI.AI";
