...
//if($_GET[ACC_ID]) {
  ...
  @-webkit-keyframes blink-animation {
   to {
    ...
    $sql_block = "SELECT YEAR(TGL), ACCOUNT_ID FROM kj_f WHERE ACCOUNT_ID = '$acon_id' AND YEAR(TGL) = '2016'";

    $hasil_block = mysqli_query($mysqli, $sql_block);
	
	$row = mysqli_num_rows($hasil_block);
	
	if($row >= 1){
        echo "<script>alert('MODERN MARKET ini sudah membuat KJ tahun 2016, anda bisa mengeditnya di Halaman HASIL KJ');
