...
<div align="center"><strong>Admin User</strong>&nbsp;&nbsp;&nbsp;<a href="home_admin.php" class="ui-btn ui-btn-inline ui-icon-back ui-btn-icon-notext ui-corner-all">No text</a></div>
<?php
  ...
  for($i=0;$i<=$jumlah;$i++) {  
	   //$sql_del = "DELETE FROM user WHERE ID = '$pilih[$i]'";
	   $sql_del = "UPDATE user SET FLAG = '0' WHERE ID = '$pilih[$i]'";
...
<form action="" method="post">
   <table  border="0" cellpadding="1" cellspacing="0" align="left">
      <tr>
         <td><?php //echo "<a href='Insert_user.php' data-role='button' data-ajax='false' target='_parent'>Add User</a>";?></td><td></td>
         <td><input type="submit" value="Hapus" name="HAPUS" onClick="return confirm('Are you sure you want to delete this item?');"/></td><td>&nbsp;&nbsp;</td><td><?php echo "<a href='../logout.php' data-role='button' data-ajax='false' target='_parent'>Log Out</a>";?></td>
      </tr>
       <!--tr>
   ...
   <thead>
   <tr><th></th><th>USER</th><th>REGIONAL</th><th>NAMA</th><th>ACTION</th></tr>
   </thead>
    				<?php
                    $sql_kj = "SELECT * FROM `user` WHERE NAMA <> '' and FLAG ='1'";
                    $hasil_kj = mysqli_query($mysqli, $sql_kj);

                    while ($data_kj = mysqli_fetch_array($hasil_kj)) {
						$triwulan = $data_kj['USER'];
						$ins = $data_kj['REGIONAL'];
						$nama = $data_kj['NAMA'];
				    echo "<tr><td align='center'><input type='checkbox' value='$data_kj[ID]' name='PILIH[]'></td>
					<td>$triwulan</td><td>$ins</td><td>$nama</td>
					<td align='center'><a href='edit_user.php?ID=$data_kj[ID]'   target='_parent' class='okl'  >EDIT</a></td></tr>";
					}
					?>
