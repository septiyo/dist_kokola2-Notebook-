				...
				for($e=0;$e<=$rangeDay;$e++) {
					echo "<td align='right'>";
						...
						$sqljmlorder = "
										...
										a.USERID,
										a.ACCOUNT_ID, 
										SUM(b.JML_ORDER) AS TOTAL_ORDER,
										c.NAMA
					...
					if($datajmlorder['TOTAL_ORDER']=='') {
						...
						$sql_total_produk = "	SELECT	c.NAMA_PRODUK,
													b.JML_ORDER															
												FROM
													order_distributor a,
													order_detail b,
							...
							$qty = $data_total_produk['JML_ORDER'];
			...
			<tr>
				...
				for($i=0; $i<=$rangeDay; $i++) {							
					...
					$sqltotal = mysqli_query($mysqli, "SELECT 
						a.TGL,
						SUM(b.JML_ORDER) AS TOTALAN																	
						FROM 
						order_distributor a, 
						order_detail b 																	
						WHERE a.TGL LIKE '$tgljadi%' AND a.ID_ORDER = b.ID_ORDER;");	
