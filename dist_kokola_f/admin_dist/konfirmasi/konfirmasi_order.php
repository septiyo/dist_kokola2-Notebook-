               ...
               // error_reporting(0);
                ...
                <table id="example" class="stripe row-border order-column" cellspacing="0" width="100%">
                   ...
                   /* $sql = "SELECT order_distributor.TGL,
					...
					$sql = "SELECT order_distributor_f.TGL,
					order_distributor_f.FLAG,
					order_distributor_f.USERID,
                    order_distributor_f.ID_ORDER,
					user_f.KOTA,
					user_f.NAMA,
					order_distributor_f.CATATAN2,
                    order_distributor_f.SBG,

					order_distributor_f.ACCOUNT_ID,
					SUM(order_detail.JML_ORDER) AS total
					FROM user,order_distributor_f,order_detail
					WHERE order_distributor_f.ACCOUNT_ID = user.ACCOUNT_ID
			        AND DATE(order_distributor_f.TGL) <= '$today_database'
					AND order_distributor_f.ID_ORDER = order_detail.ID_ORDER
					AND order_detail.FLAG = 1
					AND order_distributor_f.FLAG = 1

					GROUP BY order_distributor_f.ACCOUNT_ID,order_distributor_f.FLAG,order_distributor_f.ID_ORDER
					ORDER BY order_distributor_f.TGL DESC";
...
<div style="text-align:center"><h2>SUDAH TERKONFIRMASI</h2></div>
 <table id="example2" class="stripe row-border order-column" cellspacing="0" width="100%">
					...
					/*$sql3 = "SELECT order_confirm.TGL_CONFIRM,
							 ...
							 $sql3 = "SELECT order_confirm_f.TGL_CONFIRM,
                             user_f.NAMA,
                             user_f.KOTA,
		                     order_confirm_f.ACCOUNT_ID,
							 order_confirm_f.CATATAN2,
                             order_confirm_f.SBG,
                             SUM(order_confirm_f.JML_ORDER) AS jml_confirm
                             FROM order_confirm_f,`user_f` 
                             WHERE DATE(order_confirm_f.TGL_CONFIRM) <= '$today_database'
                             AND order_confirm_f.ACCOUNT_ID = user.ACCOUNT_ID
							 GROUP BY order_confirm_f.ACCOUNT_ID,order_confirm_f.CATATAN2
							 ORDER BY order_confirm_f.TGL_CONFIRM DESC";
...
<div style="text-align:center"><h2>SISA ORDER</h2></div>
 <table id="example3" class="stripe row-border order-column" cellspacing="0" width="100%">
					...
					$sql2 = "SELECT user_f.NAMA,
        user_f.KOTA,
        user_f.ACCOUNT_ID,
		user_f.ID,
        order_kirim_wd_f.ACCOUNT_ID,
        order_kirim_wd_f.periode2,
        SUM(qty) AS total_order,
		order_kirim_wd_f.flag
        
        FROM user_f,order_kirim_wd_f
        WHERE user_f.ACCOUNT_ID = order_kirim_wd_f.ACCOUNT_ID
        AND order_kirim_wd_f.periode2 <= '$today_database'
        AND order_kirim_wd_f.flag IN ('1','3','4','5','6')
        GROUP BY order_kirim_wd_f.ACCOUNT_ID,order_kirim_wd_f.flag";
//AND order_kirim_wd_f.flag = '1'
