...
$(document).ready(function(e) {
    var oTable = $("#tb_kj").DataTable({		
		...
		"createdRow": function ( row, data, index ) {
			...
			if ($('td', row).eq(10).text() > 0) {
				$('td', row).eq(10).parents('tr').addClass('sesuai_kj');
			}
			else {
				$('td', row).eq(10).parents('tr').addClass('tanpa_kj');;
			}
			...
			$('td', row).eq(9).css("border-right","2px solid #999");
			//$('td', row).eq(10).css("color","#063");
		...
		"footerCallback": function ( row, data, start, end, display ) {
			...
			var total_kj = api.column( 10 ).data().reduce( function (a, b) {
				return Number(a) + Number(b);
			}, 0 );
			...
			$(api.column(10).footer()).addClass("angka").html(format_digit(total_kj));
		}
	});
});
   	...
   	<div id="lebar"></div>
   	  <table id="tb_kj" class="display cell-border compact" cellspacing="0" width="100%">
       	  <thead>
           	  ...
           	  <tr>
               	  <th style="min-width:50px;">KJ</th>
                  <th style="min-width:50px;">Real</th>
                  ...
                  <th style="min-width:50px;">Real</th>
                  <th style="min-width:50px;">Sisa</th>
                  <th style="min-width:70px;">Forecast</th>
              </tr>
          </thead>
          <tfoot>
           	  <tr>
