...
$('#example').on( 'draw.dt', function () {
   ...
   //myFunction();
		...
		//"initComplete": function(settings, json) {
                   // alert( 'DataTables has finished its initialisation.' );
					//$("#MOJI").html(siop.column( 1, {page:'current'} ).data().sum());
					//bbb();
                //  },
 ...
 //alert( value );
               ...
               //"scrollY": 300,
				...
				//$('#example').on( 'draw.dt', function () {
                //alert(siop.column( 1, {page:'current'} ).data().sum() );
		       // $("#MOJI").html(siop.column( 1, {page:'current'} ).data().sum());
				//$("#TOTKJ").html(siop.column( 1, {page:'current'} ).data().sum());
                     // } ); 
					  
			 $('<k>Klik untuk sum jumlah KJ</k>')
        .prependTo( '#KOBE3' )
        .on( 'click', function () {
			$("#MOJIl").html(siop.column( 1, {page:'current'} ).data().sum());
			//alert ('dfd');
            alert('Jumlah KJ setelah search = '+siop.column( 1, {page:'current'} ).data().sum());
        } );
  ...
  //});
    ...
    <div data-role="page" id="page1" data-theme="a">
        <div data-role="header">
		   ...
		   echo $_SESSION[NAMA];?></h2><a href="report_kirim_now.php" target="_parent" class="ui-btn ui-btn-inline ui-icon-back ui-btn-icon-notext ui-corner-all">No text</a>
        </div>
<!-- dialog so-->
...
<div id="popup-tgl" data-role="popup" data-theme="a" ></div>
       ...
       <!-- <div id="KOBE" class="ui-btn ui-btn-inline"></div>-->
        <div id="MOJI"></div>
...
<form id="formx" method="post" action="save_pengiriman.php" data-ajax="false">
       <table id="example" class="cell-border"   cellspacing="0" width="100%">
        ...
        <tr class='titelku'>
    		...
    		<th style='min-width:70px' man='1'>Jumlah</th>
 ...
 ACCOUNT_ID = '$accid' and month(periode2) ='$bulanku' and year(periode2) = '$yearku' group by tglx;";
		...
		$dateku = $data9['dateku'];
			//for($i=0; $i<$jumHari; $i++){
				...
				echo "<th style='min-width:50px' man='$koj'><u><a href='#popup-tgl' data-rel='popup' class='SIMKU' data-position-to='window' class='ui-btn ui-corner-all ui-shadow ui-btn-inline ui-icon-check ui-btn-icon-left ui-btn-a' data-transition='slideup' data-mini ='true' mana='$hj' lion='$smk' mios='$dateku'> $smk</a></u></th>";
				$hj++;
				$koj++;
				}
				?>
...
//KK.BULAN1,KK.BULAN2,KK.BULAN3, KK.NAMA_DIST,KK.ACCOUNT_ID,  ID, NAMA_PRODUK, ITEM_CODE FROM m_produk as CC 
 ...
 YEAR(TGL) as THN,ID_PRODUK, BULAN1,BULAN2,BULAN3, NAMA_DIST, TRIWULAN, ITEM_CODE  from kj
...
?>
                        ...
                        <th width='50' align='center' id='hrg' class='kost' >Total</th>
