...
WHERE
  ...
  `order_detail`.`ID_ORDER` = `order_distributor`.`ID_ORDER` AND
   DATE(`order_distributor`.`TGL`) = '$tgl_d' AND
  ...
  `order_distributor`.`ACCOUNT_ID` = '$account_id'";
                   ...
                   /* if(isset($_POST[TAMPIL_SEMUA])) {

                        $tgl_jos = $_POST[TGL_JOS];
                        $acc_jos = $_POST[ACC_JOS];
                        $id_order_jos = $_POST[ID_ORDER_JOS];
...
WHERE
  push_item.`item_code` = `order_detail`.`ITEM_CODE`  AND
  `order_detail`.`ID_ORDER` = `order_distributor`.`ID_ORDER` AND
   DATE(`order_distributor`.`TGL`) <= '$tgl_jos' AND
  `order_detail`.`ID_ORDER` = '$id_order_jos' AND
  `order_distributor`.`ACCOUNT_ID` = '$acc_jos'";
