...
<script src="notjs/notify.js"></script>
<script src="notjs/notify-metro.js"></script>
...
<link rel="stylesheet" href="notjs/notify-metro.css">
...
<script>
    ...
    function notify(style) {
        $.notify({
            ...
            image: "<img src='images/kokola.png' height='32' width='32'>"
        }, {
            style: 'metro',
            className: style,
            autoHide: false,
...
$(document).ready(function () {
	...
	$('#bt_hapus').on("click", function(){
		$("#txtidorder").text("<?php echo(str_replace(" ", "", $_GET['IDORDER']));?>");
		//alert($('#editval').val());
	});
	
	$('#cfhapus').on("click", function(){
		
		var idod = "<?php echo $_GET['IDORDER']; ?>";
		var acid = "<?php echo $_GET['ID']; ?>";
		...
		//alert(idod+"-"+acid);
		 $.post("crud_konfirmasi.php", dataq, function(data) {
			...
			window.location = 'konfirmasi_orderx.php';
		});
		
		
	});

    /*$("#TAMPIL_SEMUA").on('click',function(){
...
if(isset($_POST['CONFIRM'])){	
	...
	//if($jos == "1"){
	   ...
	   $id_con = "";
   	while($n < $jumlah_qty ){
		if(($qty_confirm[$n] != "") && ($qty_confirm[$n] != "0")) {
...
<div data-role="page"  id="page1" class="type-interior" data-theme="f">
    ...
    <div data-role="content" id="top">
        ...
        $tgl_d = $_GET['TGL_D'];
	    $tgl_d = str_replace("%", " ", $tgl_d); 

         //echo "<script>alert('+$tgl_d+');</script>";
...
<div class="element">
    ...
    <!--div  id="MKI" class='NITAS' >rhyreyh</div-->
        ...
        <table border="0" width="100%">
            ...
            <!--tr>
                <td>
                    <form method="POST" action="detail_konfirmasi.php">


                        <input type="text" name="TGL_JOS" value="<?php echo $today_database;?>">
                        <input type="text" name="ACC_JOS" value="<?php echo $account_id;?>">
                        <input type="text" name="ID_ORDER_JOS" value="<?php echo $id_order;?>">
                        <input type="submit" id='TAMPIL_SEMUA' value="Tampilkan Semua Data" name="TAMPIL_SEMUA" data-role='button' class='ui-btn ui-btn-inline' data-theme='f'>
                    </form>
                </td>
        ...
        <form id='CONFIRM_JOS'>
			<table id="example" class="stripe row-border order-column" cellspacing="0" width="100%">
                <thead>
                    <tr>
                    	<td align="center" colspan="4"><h3><?php echo $nama." ".$kota." [".$account_id."]";?></h3></td>
                    </tr>
                    <tr>
  ...
  push_item.`item_name` AS NAMA_PRODUKx,
...
WHERE
  push_item.`item_code` = `order_detail`.`ITEM_CODE`  AND
    push_item.`item_code` = `m_produk`.`ITEM_CODE`  AND
  `order_detail`.`ID_ORDER` = `order_distributor`.`ID_ORDER` AND
   
   `order_distributor`.`TGL` LIKE '%$tgl_d%' AND
  `order_detail`.`ID_ORDER` = '$id_order' AND
  `order_distributor`.`ACCOUNT_ID` = '$account_id'";

                     //echo $sql;
                        //exit;  LEFT JOIN kubikasi ON kubikasi.`ITEM_CODE` = push_item.`item_code` 
	}
					...
					if($jos == 4) {
		              ...
		              $sql = "SELECT a.TGL_ORDER,
						...
						/*	 $sql = "SELECT order_confirm.TGL_CONFIRM AS TGL_ORDER,
                                    ...
                                    AND DATE(order_confirm.TGL_CONFIRM) = '$tgl_d'
                    ...
                    while($data = mysqli_fetch_assoc($hasil)){
						...
						/*if($jos == '4'){
							 ...
							 $kubikasi_total2 = number_format($data['KUBIK'], 3);
...
/*	                        echo "<td align='center'><input type='text' value='$data[qty]' class='QTY_CONFIRM' name='QTY_CONFIRM[]'> </td>";	*/
					    ...
					    echo "<td align='center'><div class='KUBIKASI_CONFIRM'>$jml_kubik</div>";
							
							}
						echo "<input type='hidden' name='KUBIKASI_BELUM_DIOLAH[]' class='nilai_kubik' value='$jml_kubik'>
                      <input type='hidden' name='KUBIKASI_DATABASE[]' id='KUBIKASI' class='KUBIKASI' value='$kubikasi_total2'>
                            <input type='hidden' name='KUBIKASI_OLAHAN[]' id='KUBIKASI_OLAHAN' class='KUBIKASI_OLAHAN'>
                ...
                <input type="hidden" name="ACC" class='ACC' value="<?php echo $account_id;?>" />
				<input type="hidden" name="SBG" class='SBG' value="<?php echo $sbg;?>" />
				<input type="hidden" name="CATATAN2"  class='CATATAN2' value="<?php echo $catatan2;?>" />
                <input type="hidden" name="TGL_ORDER" class='TGL_ORDER' value="<?php echo $tgl_d;?>" />
                
 <input name="ID_ORDER" id="un" class="ID_ORDER" value="<?php echo $id_order;?>"  data-theme="a" type="hidden">
                <?php 
				  ...
				  //if($hasil){
					  ...
					  //echo "<input type='submit' name='CONFIRM' value='CONFIRM' id='CONFIRM' />";
					  echo "<input type='button' name='CONFIRM' value='CONFIRM' id='CONFIRM' />";
					  
					  }?>

		
	
		<a href="konfirmasi_orderx.php" data-role="button" data-ajax="false">Back</a>
        <?php
         if($jos == 1){
		     echo "<a href='#popupdel' data-role='button' id='bt_hapus' data-rel='popup' data-position-to='window'>HAPUS</a>";
         }
		 ?>
        <!-- <a href="konfirmasi_order.php" data-role="button" data-ajax="false">Back</a> -->
        <!-- tambaahan -->
		
		<!--Hapus Order Belum Terkonfirmasi Form -->
		<div data-role="popup" id="popupdel" data-theme="a" data-overlay-theme="a" class="ui-corner-all" data-dismissible="false">
			<div data-role="header">
				<h1>Hapus Order Belum Terkonfirmasi</h1>
			</div>
			<div role="main" class="ui-content">
				<h3 class="ui-title">Apakah anda yakin ingin menghapus order <span id="txtidorder"></span>?</h3>
				<center>
				<a href="#" id="cfhapus" name="cfhapus" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b" data-rel="back">HAPUS</a>
				...
				<table>
					<tr>
						<td align="center" colspan="9">
							<h4 align="center">Distributor  : <?php echo $nama; ?>[<?php echo $account_id;?>]</h4>
  ...
  /* $sql_produk = "SELECT * FROM												
   ...
   /*tambah Order*/
					...
					$sql_produk = "SELECT * FROM												
  (SELECT  push_item.item_name AS NAMA_PRODUK2, 
           push_item.item_code AS ITEM_CODE, 
           (panjang*lebar*tinggi/1000000000) as KUBIK,
           m_produk.KATEGORI,
           m_produk.KET,
		   m_produk.NAMA_PRODUK AS NAMA_PRODUK              
   FROM push_item
   LEFT JOIN `push_harga`ON `push_item`.`item_code` = `push_harga`.`ITEM_CODE`
   LEFT JOIN `push_distributor` ON push_distributor.`PRICEGROUP_CODE` = push_harga.`PRICEGROUP_CODE`
   ...
   WHERE `push_distributor`.`ACCOUNT_ID` = '$account_id'
     ...
     WHERE KESATU.ITEM_CODE NOT IN (SELECT push_item.`item_code` AS ITEM_CODE
          ...
          order_distributor.`ID_ORDER` = `order_detail`.`ID_ORDER`
          AND order_detail.`ITEM_CODE` = `push_item`.`item_code`
          AND DATE(`order_distributor`.TGL) <= '$TGL_D'
    ...
    AND KESATU.ITEM_CODE NOT IN(SELECT ITEM_CODE FROM m_produk WHERE UPPER(KET) LIKE '%Discontinue%');";
					...
					while ($data_produk = mysqli_fetch_assoc($hasil_produk)) {
						...
						echo "<td align='left'>$data_produk[NAMA_PRODUK]
							<input type='hidden' value='$data_produk[ITEM_CODE]' name='ITEM_CODE_TAMBAH_ORDER[]' class='ITEM_CODE2'>
							</td>";
		echo "<td align='left'><input type='text' name='QTY_TAMBAH_ORDER[]' id='QTY' class='QTY' placeholder='...' ></td>";
					    echo "<td align='center'><div class='KUBIKASI'></div>
							<input type='hidden' name='KUBIKASI[]' value='$data_produk[KUBIK]' id='KUBIKASI' class='KUBIKASI' readonly>
							<input type='hidden' name='KUBIKASI_TAMBAH_ORDER[]' id='KUBIKASI2' class='KUBIKASI_TAMBAH_ORDER' readonly>
							</td>";
					...
					/*	echo "<tr>";
						echo "<td align='left'>$data_produk[NAMA_PRODUK]
					<input type='hidden' value='$data_produk[ITEM_CODE]' name='ITEM_CODE_TAMBAH_ORDER[]' id='QTY' class='QTY'>
							</td>";
						
	echo "<td align='left'><input type='text' name='QTY_TAMBAH_ORDER[]' id='QTY' class='QTY_TAMBAH_ORDER' placeholder='...' ></td>";
						echo "<td align='center'><div class='KUBIKASI'></div>
							<input type='text' name='KUBIKASI[]' value='$data_produk[KUBIK]' id='KUBIKASI' class='KUBIKASI' readonly>
	<input type='text' name='KUBIKASI_TAMBAH_ORDER[]' id='KUBIKASI2' class='KUBIKASI_TAMBAH_ORDER' readonly>
							</td>";
				...
				<table align="center" width="300">
					<tr>
						<td colspan="5" align="center">		
							...
							<input type="hidden" name="SBG" value="<?php echo $sbg;?>" />
				            <input type="hidden" name="CATATAN2" value="<?php echo $catatan2;?>" />
							<!--<input type="submit" value="SAVE" name="SAVE" id="SAVE">-->
                            <input type="button" value="SAVE" name="SAVE" class="SAVE">
                            <br>
...
$(document).ready(function(e) {
		...
		$(".ITEM_CODE").each(function(index, element) {
               if ($(element).val()!=="") {				
				arr.push({
					...
					QTY_CONFIRM:$(element).parents("tr").find(".QTY_CONFIRM").val(),
					KUBIKASI:$(element).parents("tr").find(".KUBIKASI").val(),
					KUBIKASI_OLAHAN:$(element).parents("tr").find(".KUBIKASI_OLAHAN").val()
	...
	$.post('action_confirm_pusat.php', {STR: arr, ACC: ACC, SBG: SBG, CATATAN2: CATATAN2, ID_ORDER: ID_ORDER, TGL_ORDER: TGL_ORDER}, function(res){
			
			
			//alert(res[0].status);
			
			var resx = jQuery.parseJSON(res);
			
			//alert(resx);

              		if(resx.status == "sukses"){
						
						//notify("white");
						//$.notify("Konfirm Berhasil..!", "success");
						 /*alert('Input Berhasil');*/
									...
									if (Notification.permission !== "granted")
										...
										//if((acc_id != null) && (acc_id != '0')){
											...
											var notifikasi = new Notification('Confirm Berhasil', {
											icon: 'kokola.png',
											body: "Konfirmasi Order Nomor = "+ ID_ORDER +" Berhasil..!"
											});
											
											notifikasi.onclick = function () {
												window.open("konfirmasi_orderx.php");      
											};
						 ...
						 //alert("Error on query!");
									...
									if (Notification.permission !== "granted")
										Notification.requestPermission();
											...
											var notifikasi = new Notification('Confirm Gagal', {
											icon: 'kokola.png',
											body: "Mohon untuk cek Koneksi anda..!"
											});
...
$('.SAVE').on("click", function(){
	    ...
	    var arr2 = [];
		var j = 0;
		$(".ITEM_CODE2").each(function(index, element) {
               if ($(element).val()!=="") {				
				arr2.push({
					ITEM_CODE2:$(element).val(),
					QTY_TAMBAH_ORDER:$(element).parents("tr").find(".QTY").val(),
					KUBIKASI:$(element).parents("tr").find(".KUBIKASI").val(),
					KUBIKASI_TAMBAH_ORDER:$(element).parents("tr").find(".KUBIKASI_TAMBAH_ORDER").val()
		...
		/*{STR: arr, STR2: arr2, ACC: ACC, SBG: SBG, CATATAN2: CATATAN2, ID_ORDER: ID_ORDER, TGL_ORDER: TGL_ORDER},*/
		
		$.post('action_confirm_pusat_tambah.php', {STR: arr, STR2: arr2, ACC: ACC, SBG: SBG, CATATAN2: CATATAN2, ID_ORDER: ID_ORDER, TGL_ORDER: TGL_ORDER}, function(res){
...
$("tr td .QTY_CONFIRM").keyup(function(e) {
	...
	$(this).parents("tr").find(".KUBIKASI_CONFIRM").html(sub_kubikasi.toFixed(3));//untuk tampilan saja
	$(this).parents("tr").find("#KUBIKASI_OLAHAN").val(sub_kubikasi.toFixed(3));//untuk hidden value
...
$("tr td .QTY").on("change", function(){
	...
	var total_kubikasi = Number(kubikasi_tambah * qty_tambah);
   ...
   // var kolom_lain =  Math.round(Number((forcast - bulan1) ));
	...
	$(this).parents('tr').find('#KUBIKASI2').val(total_kubikasi.toFixed(3));
	$(this).parents('tr').find('.KUBIKASI').html(total_kubikasi.toFixed(3));
...
function hitung_total() {
	...
	$("#kubikasi_total").html(kubikasi.toFixed(3));
...
function hitung_baru() {
	...
	$("#MKO").html("QTY : "+jml_qty.toFixed(3));
	...
	$("#MKJ").html("KUBIKASI : "+jml_kubik.toFixed(3));
	/*var jml_harga = 0;
...
/*alert (harga_dua);*/
$("#MKO").html("QTY : "+jml_qty_dua);
$("#MKJ").html("KUBIKASI : "+jml_kubik_dua.toFixed(3));	
...
if(isset($_POST['SAVE'])) {
	...
	while($i < $jumlah_qty_tambah_order){
		...
		if(($qty_tambah_order[$i] != "") && ($qty_tambah_order[$i] != "0")){
