			  ...
			  var max = '';
                $(document).ready(function () {
					...
					setInterval(function() {
					       ...
					       $.post('cek_notif.php', {max: max}, function(res){
							   
							   //alert(JSON.stringify(res));
						
							   
							   var jos = $.parseJSON(res);
						   	   var acc_id = jos[0].ACC;
							   var oke =    jos[0].TGL;
							   var id_order =  jos[0].ID_ORDER;
							    max = jos[0].TGL_MAX_DATABASE;
							  ...
							  // alert(max);
									...
									if (Notification.permission !== "granted")
										...
										if((acc_id != null) && (acc_id != '0')){
											...
											var notifikasi = new Notification('PO Baru', {
											icon: 'kokola.png',
											body: "ID Order = "+ id_order +" Account ID = " +acc_id +" Tanggal =" +oke 
											});
                    ...
                    $("div.toolbar3").html('<b><input type="text" id="cari3" placeholder="Cari"></b>');
                    $("#cari3").keyup(function(e) {
                        cari_table3();
                    });
                    ...
                    $.post("get_konfirmasi_orderx.php",function(response){
                        ...
                        $.each(arr, function (i, item) {
							...
							if(item.FLAG == 1){
							   ...
							   plat = "<img src='win.png' height='30px' width=30px>";	
							}
							else if((item.FLAG == 8) && (item.STATUS_APR == 0)){
								   
														
										plat = "<img src='no.png' height='30px' width=30px>";
							     
							}
							else if((item.FLAG == 8) && (item.STATUS_APR == 1)){
								   
														
										plat = "<img src='an.png' height='30px' width=30px>";
                            ...
                            trHTML += '<tr>' +
                                         ...
                                         '<td align=center><a href="detail_konfirmasi.php?IDORDER='+ item.ID_ORDER +'&ID='+ item.ACC_ID +'&NAMA='+item.NAMA+'&KOTA='+item.KOTA+'&USERID='+item.USERID+'&TGL_D='+item.TGL_D+'&JOS=1" data-ajax="false" target="_parent" style="text-decoration:none" class="JML_1">' + item.total + '</a></td>' +
...
//?IDORDER=$data[ID_ORDER]&ID=$data[ACCOUNT_ID]&NAMA=$data[NAMA]&KOTA=$data[KOTA]&USERID=$data[USERID]&JOS=1&TGL_D=$data[TGL_D]
                    ...
                    /*load ajax jquery json*/
                    $.post("get_sudah_konfirmx.php",function(response){
                       ...
                       // alert(JSON.stringify(response));
                        ...
                        $.each(arr, function (i, item) {
                            ...
                            trHTML += '<tr>' +
                            '<td align="center"><b>' + item.ID_ORDER +  '</b></td>' +
                            '<td align="center">' + item.TGL_ORDER +  '</td>' +
                            '<td align="center">' + item.TGL_CONFIRM +  '</td>' +
                            '<td align="center">' + item.ACC_ID +  '</td>' +
                            '<td align="center">' + item.NAMA + '</td>' +
                            '<td align="center">' + item.KOTA + '</td>' +
                            '<td align="center">' + item.SBG +  '</td>' +
                            '<td align="center">' + item.CATATAN2 + '</td>' +
...
//detail_konfirmasi.php?ID=$data3[ACCOUNT_ID]&NAMA=$data3[NAMA]&KOTA=$data3[KOTA]&USERID=$data3[ID]&JOS=4&TGL_D=$data3[TGL_D]'
				...
				setInterval(function() {
				  ...
				  $("#SUBMIT").on("click", function(e){
					  ...
					  $.post("cari_konfirm_bulan.php", {bulanx: bulanx}, function(rep){
						  
						  //alert(rep);
						  //alert(JSON.stringify(rep));
						  
						  
						  var arr = $.parseJSON(rep);
                        var trHTML = '';
                        ...
                        $.each(arr, function (i, item) {
                            ...
                            '<td align="center">' + item.SBG +  '</td>' +
                            '<td align="center">' + item.CATATAN2 + '</td>' +
                            '<td align=center><a href="detail_konfirmasi.php?IDORDER='+ item.ID_ORDER +'&ID='+ item.ACC_ID +'&NAMA='+item.NAMA+'&KOTA='+item.KOTA+'&USERID='+item.USERID+'&TGL_D='+item.TGL_D+'&JOS=4" data-ajax="false" target="_parent" style="text-decoration:none" class="JML_3">' + item.jml_confirm + '</a></td></tr>';
                        });
                        $('#tabel3').append(trHTML);

                        var jml_3 = 0;
                        $(".JML_3").each(function(index){
                             
							 //alert($(this).text());
                            jml_3 = jml_3 + Number($(this).text());


                        });

						$("#tabel3 tbody tr:eq(0)").remove();
                        $(".JUMLAH_3").html(format_digit(jml_3));
        ...
        <div data-role="page" class="type-interior" data-theme="f" id="PAGENYA">
			...
			<br><br>              
<div style="text-align:center"><h2>SUDAH TERKONFIRMASI BULAN INI</h2><br><b>
[ Note: hanya menampilkan yang sudah dikonfirmasi Bulan Ini. Untuk Bulan lain tabel dibawah.] </b>
</div>
 ...
 <table id="tabel2" class="stripe row-border order-column" cellspacing="0" width="100%">
                    ...
                    <tr>
					    <th align="center">ID Order</th>
						<th align="center">Tgl Order</th>
						<th align="center">Tgl Konfirm</th>
...
<div style="text-align:center"><h2>SUDAH TERKONFIRMASI Pilih Bulan </h2><br>
<b>[ Pilih Bulan lalu Klik Submit, tunggu sejenak hingga Laporan Muncul ]</b>

<form id='GENJOT'>	
			
<select name='BULANX' id='BULANX'>
...
<input type='submit' value='SUBMIT' name='SUBMIT' id='SUBMIT'>
 ...
 <table id="tabel3" class="stripe row-border order-column" cellspacing="0" width="100%">
                    <thead>
                    <tr>
					    <th align="center">ID Order</th>
						<th align="center">Tgl Order</th>
                    ...
                    <tr>                        
                        <th align="center" colspan="6">Total</th>
                        <th align="RIGHT" class="JUMLAH_3"></th>
