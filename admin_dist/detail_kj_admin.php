...
if($_SESSION[HAK] == "ADMIN") {
    ...
    $sql_cari_isi = "SELECT * FROM forecast WHERE TRIWULAN = '$tri' AND NAMA_DIST= '$dist'";
    echo $sql_cari_isi;
            ...
            <form method="post" action="edit_forcast.php" data-ajax="false">
                ...
                <table id="example"  class="order-column cell-border display" cellspacing="0" width="100%">
                    ...
                    <tr bgcolor="#7fffd4">
                        ...
                        <th width="80" class="header-bulan1"><?php echo $pecah[0];?> (KJ)</th>
                        <th width="80" class="header-bulan2"><?php echo $pecah[1];?></th>
                        <th width="80" class="header-bulan3"><?php echo $pecah[2];?></th>
                   ...
                   /* $sql_produk = "SELECT m_produk.`NAMA_PRODUK`,
                    ...
                    /* $sql_produk = "SELECT * FROM kj WHERE NAMA_DIST = '$dist' 
					               AND TRIWULAN = '$tri'
								   AND TGL = '$tgl'
								   AND publish = '1'								   
								   ORDER BY NAMA_PRODUK ASC"; */
								   
						$sql_produk = "SELECT *, `m_produk`.`NAMA_PRODUK` AS REV_NAMA_PRODUK
						                 FROM forecast
										INNER JOIN `m_produk` ON forecast.`ITEM_CODE` = `m_produk`.`ITEM_CODE`
										 WHERE NAMA_DIST = '$dist' 
												 AND TRIWULAN = '$tri'
												   AND TGL = '$tgl'
												  AND publish = '1'								   
                    ...
                    while ($data_produk = mysqli_fetch_assoc($hasil_produk)) {
                        ...
                        echo "<tr>";
                        echo "<th align='left' width='200'>$data_produk[NAMA_PRODUK]</th>";

                        //echo "<td align='right' class='harga'> $data_produk[HARGA]</td>";
                        echo "<td align='right' width='50'>$data_produk[BLN_AKHIR]<!--input type='text' name='bln_akhir[]' id='bln_akhir' class='bln_akhir' value='$data_produk[BLN_AKHIR]' --></td>";
                        echo "<td align='right' width='50'>$data_produk[FORECAST]<!--input type='text' name='FORECAST[]' id='FORECAST' class='FORECAST' value='$data_produk[FORECAST]'--></td>";
                        echo "<td align='center' width='50'>$data_produk[PERSEN]<!--input type='text' name='PERSEN[]' style='min-width: 30px'   id='PERSEN' class='PERSEN' value='$data_produk[PERSEN]' readonly placeholder='...' --></td>";
                        echo "<td align='right' width='80'>$data_produk[BULAN1]<!--input type='text' name='BULAN1[]' style='min-width: 100px'   id='BULAN1' class='BULAN1' value='$data_produk[BULAN1]'   --></td>";
                        echo "<td align='right' width='80'>$data_produk[BULAN2]<!--input type='text' name='BULAN2[]' style='min-width: 100px'   id='BULAN2' class='BULAN2' value='$data_produk[BULAN2]'   --></td>";
                        echo "<td align='right' width='80'>$data_produk[BULAN3]<!--input type='text' name='BULAN3[]' style='min-width: 100px'   id='BULAN3' class='BULAN3' value='$data_produk[BULAN3]'   --></td>";
...
<div data-role="collapsible" data-theme="b" data-content-theme="b">
    <h4>DELETE</h4>
    <p>Yakin akan dalete Data..?.</p>
	<?php 
	
	echo "<a href='action_delete_kj.php?DIST=$dist&TRI=$tri' data-role='button' data-ajax='false' target='_parent'>Delete</a>";
