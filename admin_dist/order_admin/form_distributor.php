...
$(document).ready(function(e) {
	...
	var tinggi = $(window).height()-t_minus-t_minus-t_minus-102;
	var jml_order = 0;
	var jml_kubik = 0;
			...
			onok: function(){
				...
				$.ajax({
					...
					success: function(hasil) {
						...
						var dataq2 = {
                                ACCOUNT_ID: $("#session").text()
                            };
						$.post("http://10.1.13.54/m_kj/server_mobile/push_notif_confirm_p.php", dataq2, function(data) {
						   //var arr = $.parseJSON(data);
						   if (data == 11) {
							   
						   } else if (data == 01){
							   alert("Notifikasi gagal dikirimkan kepada distributor.");
						   }else if (data == 10){
							   alert("Notifikasi gagal dikirimkan kepada sales.");
						   }else {
							   alert("Notifikasi gagal dikirimkan.");
						   }
						});
...
$(document).on("pageinit", function() {
	var xTable = $("#tb_data").dataTable({		
		...
		"bInfo": false,
		"ajax": {
            "url": "get_order.php?acc_id=" + QueryString.acc_id,
            "dataSrc": "data"
        },
		"language": {
            "lengthMenu": "Display _MENU_ records per page",
...
$sqlDist = "select ACCOUNT_ID, ACCOUNT_NAME, ACCOUNT_ADDRESS1, ACCOUNT_CITY_ID1
	from push_distributor where ACCOUNT_ID = '".$account_id."'";
$myDist = mysql_query($sqlDist);
$jmlAcc = mysql_num_rows($myDist);
if ($dataAcc = mysql_fetch_assoc($myDist)) {
	$account_id = $dataAcc['ACCOUNT_ID'];
	$nama = $dataAcc['ACCOUNT_NAME'];
	$kota = $dataAcc['ACCOUNT_CITY_ID1'];
...
<div data-role="page" data-theme="a">
	<div data-role="header" data-position="fixed" data-tap-toggle="false">
        <div class="ui-btn-left" data-role="controlgroup" data-type="horizontal">
        	<a id="btn_order" href="../order_distributor.php" data-role="button" data-icon="home" data-ajax="false" data-iconpos="notext">Home</a>
            <a id="btn_order" href="data_kj.php?acc_id=<?php echo $account_id;?>" data-role="button" data-icon="grid" data-ajax="false" data-mini="true">KJ</a>
        </div>
	...
	<div data-role="content">
		<div id="identitas">
            ...
            <div data-role="popup" id="myPopup" class="ui-content" data-theme="r" data-overlay-theme="r" data-corners="false">
        		<a href="#" data-rel="back" class="ui-btn ui-corner-all ui-shadow ui-btn ui-icon-delete ui-btn-icon-notext ui-btn-right">Close</a>
						...
						$sql_produk = "select MM.ID, YO.ITEM_CODE, YO.item_name, MM.KATEGORI,
							...
							OK.qty, SHIT.KUBIK from push_item YO
								...
								left join kubikasi SHIT
									on YO.ITEM_CODE = SHIT.ITEM_CODE
								where YO.ITEM_CODE not in
								(select ITEM_CODE from forecast
									where ACCOUNT_ID = '".$account_id."'
            ...
            <div id="div_order">
                ...
                <table id="tb_order" class="display cell-border compact" cellspacing="0" width="100%">
                    <thead>
                    	<tr>
                            ...
                            <span style="margin-right:15px;display:inline-block;">
                                ...
                                <span id="session" style="display:none"><?php echo $account_id;?></span>
                    ...
                    <tfoot>
                        ...
                        <tr>
                        	<?php
							$sqlBlink = "select KALIMAT from kata_sakti";
							$myBlink = mysql_query($sqlBlink);
							if ($dataBlink = mysql_fetch_assoc($myBlink)) {
							?>
                        	<th colspan="7"><div class="blink"><?php echo $dataBlink['KALIMAT'];?></div></th>
                            <?php
							} else {
                            ?>
                            <th colspan="7">&nbsp;</th>
					...
					$sql_order = "select MM.ID, AA.ITEM_CODE, AA.item_name,
						...
						(select ID_PRODUK, COALESCE(FORECAST,0) as FORECAST, ITEM_CODE,
							COALESCE(BULAN1,0) as BULAN1,
							COALESCE(BULAN2,0) as BULAN2,
							COALESCE(BULAN3,0) as BULAN3 from forecast
