	...
	if($jos == "1"){
   	...
   	while($n < $jumlah_qty ){
		if($qty[$n] != "") {
			...
			/*cari dulu apa di tanggal yang sama sudah ada account_idnya*/
			$sql_cek_confirm = "SELECT * FROM order_confirm WHERE DATE(TGL_CONFIRM) = '$today_database' AND ACCOUNT_ID = '$account_idne'";
			
			//echo $sql_cek_confirm;
			
			$hasil_cek = mysqli_query($mysqli, $sql_cek_confirm);
		    $rowcount = mysqli_num_rows($hasil_cek);
			...
			if($rowcount >= 1){
				...
				ID_CONFIRM = '$data_cek[ID_CONFIRM]',
				...
				$cari_total = "SELECT SUM(JML_ORDER) as total, ID_CONFIRM FROM order_confirm WHERE ACCOUNT_ID = '$account_idne' 
				                 AND DATE(TGL_CONFIRM) = '$today_database'";
				$hasil_total = mysqli_query($mysqli, $cari_total);
				$data_total = mysqli_fetch_assoc($hasil_total);
				$total = $data_total[total];
				$id_con = $data_cek[ID_CONFIRM];
				
				//echo $cari_total;
			
									
			/*update confrim total*/
			
			  $update_confirm_total = "UPDATE order_confirm_total SET JML_CONFIRM = '$total' WHERE ID_CONFIRM = '$id_con'";
					
					//echo $input_ke_order_total;
				
				$hasil_confirm_total = mysqli_query($mysqli, $update_confirm_total);
						//exit;		
			...
			if($rowcount == 0){
				...
				$cari_total = "SELECT JML_ORDER  FROM order_confirm WHERE ACCOUNT_ID = '$account_idne' AND DATE(TGL_CONFIRM) = '$today_database'";
				$hasil_total = mysqli_query($mysqli, $cari_total);
				$data_total = mysqli_fetch_assoc($hasil_total);
				$total = $data_total[JML_ORDER];
				
					//goto bawah;				
			/*insert confrim total*/
					
				$input_ke_order_total = "INSERT INTO order_confirm_total SET  
				ID_CONFIRM = '$rid',
				TGL_ORDER = '$tgl_order[$n]',
				ACCOUNT_ID = '$account_idne',
		    	JML_CONFIRM = '$total',
				...
				$hasil_ke_order_total = mysqli_query($mysqli, $input_ke_order_total);
    ...
    <div  id="MKI" class='NITAS' >rhyreyh</div>
        ...
        <form method="POST" action="detail_konfirmasi.php" data-ajax="false" id="FORM_CONFIRM">
			<table id="example" class="stripe row-border order-column" cellspacing="0" width="100%">
                <thead>
                    <tr>
                    	<td align="center" colspan="6"><h3><?php echo $nama." ".$kota;?></h3></td>
                    </tr>
                    <tr>
...
FROM
  `order_distributor`,
  `order_detail`
  LEFT JOIN `kubikasi` ON `order_detail`.`ITEM_CODE` = `kubikasi`.`ITEM_CODE`,
  `user`,
  `m_produk`
WHERE
  `order_detail`.`ID_ORDER` = `order_distributor`.`ID_ORDER` AND
  `user`.`ACCOUNT_ID` = `order_distributor`.`ACCOUNT_ID` AND
  `order_detail`.`ITEM_CODE` = `m_produk`.`ITEM_CODE` AND
  `order_distributor`.`TGL` LIKE '$today_database%' AND
  ...
  `order_distributor`.`ACCOUNT_ID` = '$account_id'";
					...
					$sql = "SELECT user.NAMA,
						...
						if($jos == 4) {
							...
							$sql = "SELECT order_confirm.TGL_CONFIRM AS TGL_ORDER,
                                    ...
                                    FROM order_confirm,m_produk
                                    WHERE m_produk.ITEM_CODE = order_confirm.ITEM_CODE
                                    AND DATE(order_confirm.TGL_CONFIRM) <= '$today_database'
                                   AND order_confirm.ACCOUNT_ID = '$account_id'";
								   
								   
								   //AND order_confirm.ID_CONFIRM = 'B05312'
							
							/*$sql = "SELECT order_confirm.TGL_CONFIRM AS TGL_ORDER,
                                     order_confirm.ITEM_CODE,
                                   m_produk.HARGA,
                                   m_produk.NAMA_PRODUK,
                                   order_confirm.JML_ORDER AS qty,
                                order_confirm.KUBIKASI AS KUBIK
       
                                FROM order_confirm,m_produk,user 
                             WHERE DATE(order_confirm.TGL_CONFIRM) <= '$today_database'
                             AND order_confirm.ACCOUNT_ID = user.ACCOUNT_ID
                             AND order_confirm.ITEM_CODE = m_produk.ITEM_CODE
                              AND order_confirm.ACCOUNT_ID = '$account_id'
                             GROUP BY order_confirm.ITEM_CODE,order_confirm.ID_CONFIRM";*/
                    ...
                    while($data = mysqli_fetch_assoc($hasil)){
                        ...
                        if($jos == '4'){
							...
							echo "<td align='center'><input type='text' value='$data[qty]' class='QTY_CONFIRM' name='QTY[]' readonly> </td>";	
 ...
 <input name="ID_ORDER" id="un" value="<?php echo $id_order;?>"  data-theme="a" type="hidden">
                <?php 
				  ...
				  if($hasil == false){echo "<input type='submit' name='CONFIRM' value='CONFIRM' />";}?>
                
				
				<!--penutup form aslinya-->
				<!--/form-->
...
if(isset($_POST['SAVE'])) {
	...
	//$qty          = $_POST['QTY'];
	$qty_tambah          = $_POST['QTY_TAMBAH'];
	
	$jumlah_qty   = count($qty);
	$account_idne = $_POST['ACC'];
	$userid       = $_POST['USERID'];
	$item_code    = $_POST['ITEM_CODE'];
	//$item_code_tambah    = $_POST['ITEM_CODE_TAMBAH'];
	
	$kubikasi     = $_POST['KUBIKASI2'];
	//$kubikasi_tambah     = $_POST['KUBIKASI2_TAMBAH'];
	...
	/*select dulu dari data terus input ke order_confirm ini yang tadinya sudah ada di order*/
	
	/*$cari_yang_baru_diubah = "SELECT * FROM order_detail WHERE ID_ORDER = '$id_akhir'";
	$hasil_yg_baru_diubah = mysqli_query($mysqli, $cari_yang_baru_diubah);*/
   ...
   $n=0;
	...
	/*while($data_cari_yg_diubah = mysqli_fetch_assoc($hasil_yg_baru_diubah)){
		
		   	
		$insert_confirm_tambah = "INSERT INTO order_confirm 
			    SET ID_ORDER = '$id_akhir',
				ID_CONFIRM = '$rid',
				TGL_ORDER = '$data_cari_yg_diubah[DATE_INSERT]',
				ACCOUNT_ID = '$account_idne',
				ID_PRODUK = '$data_cari_yg_diubah[ID_PRODUK]',
				ITEM_CODE = '$data_cari_yg_diubah[ITEM_CODE]',
				JML_ORDER = '$data_cari_yg_diubah[JML_ORDER]',
				KUBIKASI = '$data_cari_yg_diubah[KUBIKASI]',
				TGL_CONFIRM = '$today_database $time',
				FLAG2 = '3'";
	...
	while ($n < $jumlah_qty) {	
		...
		if($qty[$n] != "") {	
			 $sql_order_detail = "INSERT INTO order_detail SET  ID_ORDER = '$id_akhir',
								ID_PRODUK = '$id_produk',
								JML_ORDER = '$qty[$n]',
								ITEM_CODE = '$item_code[$n]',
								KUBIKASI  = '$kubikasi[$n]',
	 ...
	 /*$update_flag_sisa = "INSERT INTO order_kirim_wd SET item_code = '$item_code[$n]',
                                                          ACCOUNT_ID = '$account_idne',
														  qty = '$qty[$n]',
														  periode2 = '$today_database',
 													      flag = '6'";
