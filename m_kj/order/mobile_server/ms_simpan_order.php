...
//echo print_r($_POST);
//$kj = isset($_REQUEST['ARR']) ? $_REQUEST['ARR'] : 'kosong';
//echo $kj[0]['kj'];
//echo $kj[1]['kj'];

$tgl_cm=isset($_REQUEST['TGL_PILIH']) ? $_REQUEST['TGL_PILIH'] : '';
if($tgl_cm==""){
    $tgl_cm=date('Y-m-d');    
}
...
//        TGL_JADWAL_KIRIM = '".$tgl_cm." ".date('H:i:s')."',    
//	CATATAN = '".$catatan."',
//	CATATAN2 = '".$catatan2."',
//	ACCOUNT_ID = '".$acc_id."',
//	SBG = '".$ck_ps."'";
...
if ($mysqOrder = mysql_query($sqlOrder)) {
	...
	$sqlHapus = "delete from order_draft where ACCOUNT_ID = '".$acc_id."'";
