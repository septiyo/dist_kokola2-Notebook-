...
for($i=0; $i<count($sen); $i++){
	...
	$values .= "(DEFAULT, '".date("Y-m-d h:i:s", time())."', '".$acd[$i]."', '".$deco['multicast_id']."', '".$deco['success']."', 
			'".$deco['failure']."', '".$deco['canonical_ids']."', '".$deco['results'][0]['message_id']."', 'NOTIF_JALAN'),";
	
		
}

echo ("Execution time: ".$time_elapsed_secs = microtime(true) - $starttime." seconds");
//file_put_contents('./log_selector_jalan.txt', $log_m.'['.date("Y-m-d", time()).']-['.date("h:i:s A", time()).']-['.$time_elapsed_secs."]\n", FILE_APPEND);
