	...
	if(mysql_num_rows($res) > 0){
		...
		$sql = "SELECT aa.*, bb.* FROM
				(SELECT *, SUM(qty) AS total_order, COUNT(*) AS jenis_item 
					...
					WHERE okw.periode2 = CURDATE() AND okw.ACCOUNT_ID = '".$acc_id[$i]."')aa,
				(SELECT uu.NAMA, uu.ID_APKEY 
					FROM distributor_kokola.user uu 
					WHERE uu.ACCOUNT_ID = '".$acc_id[$i]."') bb;";
		...
		$values .= "(DEFAULT, '".date("Y-m-d h:i:s", time())."', '".$acc_id[$i]."', '".$deco['multicast_id']."', '".$deco['success']."', 
				'".$deco['failure']."', '".$deco['canonical_ids']."', '".$deco['results'][0][$stat]."', 'NOTIF_ORDER_D_H'),";
...
$data = array('OPERAND' => 'selector_order_d_h', 'LOG' => $log_mm);
$options = array(
	'http' => array(
